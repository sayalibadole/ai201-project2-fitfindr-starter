...
def handle_query(user_query: str, wardrobe_choice: str) -> tuple[str, str, str]:
    ...
    # 4. If the agent stopped early, show the error in the first panel only.
    if session["error"]:
    ...
    # Surface any agent notes (e.g. a relaxed size filter) above the listing.
    notes = "\n".join(f"ℹ️ {n}" for n in session["notes"])
    listing_text = (notes + "\n\n" if notes else "") + "\n".join(listing_lines)
